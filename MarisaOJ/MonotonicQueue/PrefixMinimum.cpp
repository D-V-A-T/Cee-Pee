...
struct Seggs{
    ...
    Seggs(){
        ...
        for(int ii=l;r>=ii;ii++){
            int i=ii-l;
            t[id].v[i]= i ? min(t[id].v[i-1], a[ii]) : a[ii];
        ...
        vect<int> ids;
        get(L, R, ids);
        int ans=t[ids[0]].pfs.back(), nmin=t[ids[0]].v.back();
        for(int i=1;ids.size()>i;i++){
            int ix=ids[i], l=0, r=t[ix].v.size()-1, res=-1;
            while(l <= r){
                int mid=(l+r)/2;
                if(t[ix].v[mid] <= nmin)r=mid-1;
                else{
                    res=mid;
                    l=mid+1;
                }
            }if(res > -1)
                ans+=(res+1)*nmin - t[ix].pfs[res] + t[ix].pfs.back();
            else ans += t[ix].pfs.back();
            nmin=min(nmin, t[ix].v.back());
        }return ans;
    }
};
