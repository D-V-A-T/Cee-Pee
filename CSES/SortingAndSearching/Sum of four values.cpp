...
void sol(){
    ...
    map<int, pii> mp;
    for(int i=0;n>i;i++){
        for(int j=i+1;n>j && mp.size();j++){
            if(a[i].fi + a[j].fi >= x)continue;
            pii &pa = mp[x-a[i].fi-a[j].fi];
            // if(a[i].fi + a[j].fi == 5)cout << "gud\n";
            if(pa.fi != pa.se){
                cout << pa.fi << ' ' << pa.se << ' ' << a[i].se << ' ' << a[j].se;
                return;
            }
        }
        for(int j=i-1;j>=0;j--){
            mp[a[i].fi + a[j].fi] = {i+1, j+1};
        }
    }
    cout << "IMPOSSIBLE";
}
